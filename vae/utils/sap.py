...
def _r2_single_dim(x: np.ndarray, y: np.ndarray) -> float:
    """
    x: [N] single latent dim
    y: [N] age
    returns max(0, R^2)
    """
    x = x.reshape(-1, 1)
    ...
    regr.fit(x, y)
    y_pred = regr.predict(x).reshape(-1)
    r2 = r2_score(y.reshape(-1), y_pred)
    return float(max(0.0, r2))

def sap_fixed_age(factors: np.ndarray,
                  codes: np.ndarray,
                  age_dim: int = 1) ->  Tuple[float, float, float]:
    """
    factors: [N,1] ages
    codes:   [N,D] latent codes (use mu ideally)
    Returns:
      score = R2(age_dim) - max_{d!=age_dim} R2(d)
      r2_target
      r2_best_other
    """
    if factors.ndim == 2 and factors.shape[1] >= 1:
        age = factors[:, 0]
    else:
        age = factors.reshape(-1)

    N, D = codes.shape
    assert 0 <= age_dim < D, f"age_dim={age_dim} out of range for D={D}"

    r2s = np.zeros(D, dtype=np.float64)
    for d in range(D):
        r2s[d] = _r2_single_dim(codes[:, d], age)

    r2_target = float(r2s[age_dim])
    r2_best_other = float(np.max(np.delete(r2s, age_dim))) if D > 1 else 0.0
    score = r2_target - r2_best_other
    return float(score), r2_target, r2_best_other
