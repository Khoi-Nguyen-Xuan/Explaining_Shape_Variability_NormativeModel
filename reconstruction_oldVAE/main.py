...
# --- config
ROOT_SAVE = "/scratch/xuankhoi/mesh/Explaining_Shape_Variability_CALSNIC/src/DeepLearning/compute_canada/guided_vae/data/CoMA/raw/torus/models_guided_2601ogSAPfaster_VAE_valoptuna"
...
def objective(trial):
    ...
    args.w_leak = trial.suggest_float("w_leak", 1e-4, 10.0, log=True)
    ...
    args.latent_channels = trial.suggest_categorical("latent_channels", [128, 256, 384, 512])
    args.threshold = trial.suggest_categorical("threshold", [8, 12, 16, 20])
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            _, mu, log_var, *_ = model(x)  # use mu for scoring (stable)
    ...
    # ----- Standard SAP (keep for reference/logging) -----
    sap_score = sap(
        factors=ages.numpy(),
...
class LogAfterEachTrial:
    def __call__(self, study: optuna.study.Study, trial: optuna.trial.FrozenTrial) -> None:
        trials = study.trials
        torch.save(trials, "/scratch/xuankhoi/mesh/Explaining_Shape_Variability_CALSNIC/src/DeepLearning/compute_canada/guided_vae/data/CoMA/raw/torus/models_contrastive_inhib/intermediate_trials.pt")
...
study = optuna.create_study(
    ...
    sampler=sampler,
    storage="sqlite:////scratch/xuankhoi/optuna_multiobj.db",
    load_if_exists=True,
)
